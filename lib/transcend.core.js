...
(function(module) {
    ...
    _.extend(Transcend.prototype, {
         ...
         *
         * Each handler's 'initialize' method is called once.
         * Each handler's 'prepare' method is called, once for each file.
         *     The 'prepare' method receives the Transcend.File object as its parameter.
         *     Handlers can call 'ensurePrepared' to ensure that another directive handler is prepared first.
         * Each handler's 'process' method is called, once for each non-hidden file.
         * The file object's .lines array is written to the output file.
         * Each handler's 'finalize' method is called, once for each non-hidden file.
         * Each handler's 'complete' method is called once.
         */
        process: function () {
            ...
            try {
                ...
                if(e.message.indexOf('Cannot find module') !== -1 && this.configFile === 'transcend.json')
                    this.config = {};
                else {
            ...
            for(var directive in Transcend._handlers) {
                if(typeof Transcend._handlers[directive].initialize === 'function')
                    Transcend._handlers[directive].initialize.call(this);
            }
            ...
            for(var f in this.files)
            {
                ...
                for(var directive in Transcend._handlers)
                {
                    if(typeof Transcend._handlers[directive].process === 'function')
            ...
            for(var f in this.files)
            {
                ...
                if(file.hidden()) continue;
                for(var directive in Transcend._handlers) {
                    if(typeof Transcend._handlers[directive].finalize === 'function')
                        Transcend._handlers[directive].finalize.call(this, file);
                }
            }

            // Run "complete" once
            for(var directive in Transcend._handlers) {
                if(typeof Transcend._handlers[directive].complete === 'function')
                    Transcend._handlers[directive].complete.call(this);
            }
        },

        /**
         * Ensures that another directive has already had it's 'prepare' process completed.
         * @param directive The directive to prepare
         */
